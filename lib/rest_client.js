        ...
        try {
            ...
            if (e && typeof (e) == 'string' && (
                ...
                if (request_data.body.product) {
                    var urlAttribute = _.find(request_data.body.product.custom_attributes, { attribute_code: "url_key" })
                    if (retry == 1) {
                        urlAttribute.value = slugify(urlAttribute.value + "-" + request_data.body.product.sku)
                    } else if (retry == 1) {
                        urlAttribute.value = urlAttribute.value + "-" + retry
                    } else if (retry > 2) {
                        urlAttribute.value = urlAttribute.value.replace("-" + retry - 1, "-" + retry)
                    }
                    /*
                ...
                if (request_data.body.category) {
                    console.log(request_data.body.category)
                    var urlAttribute = _.find(request_data.body.category.custom_attributes, { attribute_code: "url_key" })
                    if (retry == 1) {
                        urlAttribute.value = urlAttribute.value + "-" + retry
                    } else if (retry > 1) {
                        urlAttribute.value = urlAttribute.value.replace("-" + (retry - 1), "-" + retry)
                    }
                    console.log("Category URL already in use. Retrying with slightly changed URL: " + slugify(urlAttribute.value))
                    /*request_data.body.category.custom_attributes.push({
                        "attribute_code": "url_key",
                        "value": slugify(url + "-" + retry)
                    })*/
                }
