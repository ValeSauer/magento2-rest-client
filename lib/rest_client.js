        ...
        try {
            ...
            if (e && typeof (e) == 'string' && (
                ...
                if (request_data.body.product) {
                    var url = _.find(request_data.body.product.custom_attributes, {attribute_code: "url_key"}).value
                    if (retry == 1) {
                        url = slugify(url + "-" + request_data.body.product.sku)
                    } else if (retry > 1) {
                        url = slugify(url + "-" + request_data.body.product.sku + "-" + retry)
                    }
                ...
                if (request_data.body.category) {
                    var url = _.find(request_data.body.category.custom_attributes, {attribute_code: "url_key"}).value
                    console.log("Category URL already in use. Retrying with slightly changed URL: " + slugify(url + "-" + retry))
