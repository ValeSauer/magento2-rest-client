...
module.exports.RestClient = function (options) {
    ...
    var apiCall = async function (request_data, retry = 0) {
        try {
            ...
            if (e && typeof (e) == 'string' && e.indexOf("URL key for specified store already exists.") >= 0 && retry <= MAXURLRETRIES) {
                retry++
                if (request_data.body.product) {
                    ...
                    if (retry == 1) {
                        ...
                        url = slugify(request_data.body.product.name + "-" + request_data.body.product.sku + "-" + sku)
                    }
                ...
                if (request_data.body.category) {
                    console.log("Category URL already in use. Retrying with slightly changed URL")
            ...
            else if (e && typeof (e) == 'string' && e.indexOf("Database deadlock found when trying to get lock") >= 0 && retry <= MAXURLRETRIES) {
                retry++
